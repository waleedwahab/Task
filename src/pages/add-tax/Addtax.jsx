...
const groupItemsByCategory = (items) => {
  const categories = {};
  const uncategorized = [];
  const filteredItems = items.filter(item => !item.options?.recurring?.enabled);
 filteredItems.forEach((item) => {
    if (item.category) {
      const catId = item.category.id;
      if (!categories[catId]) {
        categories[catId] = { name: item.category.name, items: [] };
      }
...
const AddTax = () => {
  ...
  const { categories, uncategorized } = groupItemsByCategory(items);
  ...
  const handleToggleAll = () => {
    ...
    formik.setFieldValue('applicable_items', items.map((item) => item.id), false);
    formik.handleSubmit(); 
  };
  ...
  const handleToggleSome = () => {
    ...
    formik.setFieldValue('applicable_items', [], false);
    formik.handleSubmit();
  };

  const handleCategoryToggle = (catItems, isChecked) => {
    const newSelected = new Set(formik.values.applicable_items);
    catItems.forEach(item => {
      if (isChecked) newSelected.add(item.id);
      else newSelected.delete(item.id);
    });
    formik.setFieldValue('applicable_items', Array.from(newSelected));
  };

  const handleItemToggle = (id) => {
    const selected = new Set(formik.values.applicable_items);
    if (selected.has(id)) selected.delete(id);
    else selected.add(id);
    formik.setFieldValue('applicable_items', Array.from(selected));
  };

  return (
    <div className='main-cont'>
    <form className="add-tax-container" onSubmit={formik.handleSubmit}>
      <div className="title">Add Tax</div>
      ...
      <input
        ...
        <img className='search-icons' src = {searchicon}/>
        </div>
        ...
        {Object.entries(categories).map(([catId, cat]) => (
          <div key={catId}>
            <label className="item category">
              <input
                type="checkbox"
                checked={cat.items.every(item => formik.values.applicable_items.includes(item.id))}
                onChange={(e) => handleCategoryToggle(cat.items, e.target.checked)}
              /> {cat.name}
            </label>
...
{uncategorized.length > 0 && (
  <div>
    <label className="item category">
      <input
        type="checkbox"
        checked={uncategorized.every(item => formik.values.applicable_items.includes(item.id))}
        onChange={(e) => handleCategoryToggle(uncategorized, e.target.checked)}
      /> Uncategorized
    </label>
    <div className="sub-items">
